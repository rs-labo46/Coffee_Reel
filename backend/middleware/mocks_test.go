...
func (m *userUsecaseMock) Refresh(ctx context.Context, token string) (usecase.AuthTokens, error) {
...
type tokenServiceMock struct {
	parseAccessTokenFunc func(string, time.Time) (usecase.AccessTokenClaims, error)
	compareCSRFTokenFunc func(string, string) error
}

func (m *tokenServiceMock) HashPassword(string) (string, error) {
	panic("unexpected TokenService.HashPassword call")
}
func (m *tokenServiceMock) ComparePassword(string, string) error {
	panic("unexpected TokenService.ComparePassword call")
}
func (m *tokenServiceMock) GenerateAccessToken(uint64, entity.UserRole, uint64, time.Time) (string, error) {
	panic("unexpected TokenService.GenerateAccessToken call")
}
func (m *tokenServiceMock) ParseAccessToken(token string, now time.Time) (usecase.AccessTokenClaims, error) {
...
func (m *tokenServiceMock) HashRefreshToken(string) string {
...
func (m *rateLimitUsecaseMock) AllowSignup(ctx context.Context, clientIP string) (usecase.RateLimitDecision, error) {
	if m.allowSignupFunc == nil {
		panic("unexpected RateLimitUsecase.AllowSignup call")
	}
	return m.allowSignupFunc(ctx, clientIP)
}
func (m *rateLimitUsecaseMock) AllowLoginIP(ctx context.Context, clientIP string) (usecase.RateLimitDecision, error) {
	if m.allowLoginIPFunc == nil {
		panic("unexpected RateLimitUsecase.AllowLoginIP call")
	}
	return m.allowLoginIPFunc(ctx, clientIP)
}
