...
func TestUserControllerSignUpValidatesThenReturnsSafeCreatedUser(t *testing.T) {
	...
	validator := &userValidatorMock{
		validateSignupFunc: func(name, email, password string) (string, string, string, error) {
	...
	users := &userUsecaseMock{
		signUpFunc: func(_ context.Context, name, email, password string) (*entity.User, error) {
			usecaseCalled = true
	...
	c, rec := newControllerContext(
		http.MethodPost,
		"/signup",
		`{"name":" Raw Name ","email":" USER@EXAMPLE.COM ","password":" password "}`,
	)
	...
	if !validatorCalled || !usecaseCalled || rec.Code != http.StatusCreated {
		t.Fatalf(
			"validator=%v usecase=%v status=%d",
			validatorCalled,
			usecaseCalled,
			rec.Code,
		)
	...
	if response.Data.ID != 10 ||
		...
		!response.Data.CreatedAt.Equal(createdAt) {
...
func TestUserControllerSignUpRejectsMalformedJSONBeforeValidation(t *testing.T) {
	...
	if response.Code != "validation_failed" ||
		response.RequestID != "request-controller" {
		t.Fatalf("response = %+v", response)
	}
}
...
func TestUserControllerLoginSuccessSetsSecureCookieContractWithoutReturningRefreshSecrets(t *testing.T) {
	...
	validator := &userValidatorMock{
		validateLoginFunc: func(email, password string) (string, string, error) {
	...
	rateLimits := &rateLimitUsecaseMock{
		allowLoginEmailFunc: func(_ context.Context, email string) (usecase.RateLimitDecision, error) {
			if email != "user@example.com" {
	...
	users := &userUsecaseMock{
		loginFunc: func(_ context.Context, email, password string) (usecase.LoginResult, error) {
			if email != "user@example.com" ||
				password != " password " {
	...
	if response.Data.AccessToken != "access-token" ||
		...
		response.Data.CSRFToken != "csrf-secret" {
		t.Fatalf("response = %+v", response)
	}

	if response.Data.User.Role != entity.RoleUser ||
		response.Data.User.Status != entity.StatusActive {
	...
	if refreshCookie.Value != "refresh-secret" ||
		!refreshCookie.HttpOnly ||
		!refreshCookie.Secure ||
		!refreshCookie.Partitioned ||
		refreshCookie.Path != "/" ||
		refreshCookie.SameSite != http.SameSiteNoneMode ||
		refreshCookie.MaxAge != cookieMaxAge ||
	...
	if csrfCookie.Value != "csrf-secret" ||
		!csrfCookie.HttpOnly ||
		!csrfCookie.Secure ||
		!csrfCookie.Partitioned ||
		csrfCookie.Path != "/" ||
...
func TestUserControllerLocalCookiesAreNotPartitioned(t *testing.T) {
	expiresAt := time.Now().
		Add(7 * 24 * time.Hour).
		Truncate(time.Second)
	...
	controller.setAuthCookies(c, usecase.AuthTokens{
	...
	for _, name := range []string{
		...
		if cookie.Partitioned {
			t.Fatalf("%s cookie Partitioned = true, want false", name)
		}

		if cookie.SameSite != http.SameSiteLaxMode {
			t.Fatalf(
				"%s cookie SameSite = %v, want %v",
				name,
				cookie.SameSite,
				http.SameSiteLaxMode,
			)
...
func TestUserControllerLoginEmailRateLimitBlocksAuthentication(t *testing.T) {
	...
	rateLimits := &rateLimitUsecaseMock{
		allowLoginEmailFunc: func(
			...
			return usecase.RateLimitDecision{
				Allowed:    false,
				RetryAfter: 1201 * time.Millisecond,
			}, nil
		},
	}
...
func TestUserControllerLoginRateLimitStorageFailureReturnsGeneric503(t *testing.T) {
	...
	if rec.Code != http.StatusServiceUnavailable ||
		strings.Contains(rec.Body.String(), redisErr.Error()) {
		t.Fatalf(
			"status=%d body=%s",
			rec.Code,
			rec.Body.String(),
...
func TestUserControllerCSRFIssuesCookieAndResponseToken(t *testing.T) {
	...
	for _, cookie := range rec.Result().Cookies() {
		if cookie.Name == refreshCookieName {
			t.Fatalf(
				"CSRF bootstrap must not issue refresh cookie: %+v",
				cookie,
			)
		}
	}
...
func TestUserControllerRefreshSuccessRotatesBothCookies(t *testing.T) {
	...
	if strings.Contains(rec.Body.String(), "new-refresh") {
		t.Fatalf(
			"refresh response leaks refresh token: %s",
			rec.Body.String(),
		)
	}
	...
	if refreshCookie.Value != "new-refresh" ||
		csrfCookie.Value != "new-csrf" {
		t.Fatal("refresh did not rotate both authentication cookies")
	}

	if !refreshCookie.Partitioned ||
		!csrfCookie.Partitioned {
		t.Fatalf(
			"rotated cookies must remain partitioned: refresh=%+v csrf=%+v",
			refreshCookie,
			csrfCookie,
		)
	}
...
func TestUserControllerRefreshReuseClearsCookiesAndReturnsGenericUnauthorized(t *testing.T) {
	...
	if strings.Contains(rec.Body.String(), "reused") {
		t.Fatalf(
			"reuse details or token leaked: %s",
			rec.Body.String(),
		)
	}
	...
	for _, name := range []string{
		...
		if cookie.Value != "" ||
			cookie.MaxAge != -1 ||
			cookie.Expires.After(time.Unix(1, 0)) ||
			!cookie.Partitioned ||
...
func TestUserControllerLogoutPassesCookieAndClearsBothCookies(t *testing.T) {
	...
	if logoutToken != "refresh-secret" ||
		rec.Code != http.StatusNoContent {
		t.Fatalf(
			"logoutToken=%q status=%d",
			logoutToken,
			rec.Code,
		)
	}
...
func TestUserControllerMeRequiresMiddlewareUserAndReturnsSafeResponse(t *testing.T) {
	...
	t.Run("current user", func(t *testing.T) {
		...
		if strings.Contains(rec.Body.String(), "hash") ||
			strings.Contains(rec.Body.String(), "token_version") {
			t.Fatalf(
				"Me response leaks auth state: %s",
				rec.Body.String(),
			)
		}
	})
...
func TestWriteErrorMapsDomainErrorsAndNeverLeaksInternalDetails(t *testing.T) {
	internalErr := errors.New(
		"pq: duplicate key constraint secret_table_key stack trace",
	)
	...
	tests := []struct {
		...
		{
			name:       "suspended",
			err:        entity.ErrUserSuspended,
			wantStatus: http.StatusForbidden,
			wantCode:   "user_suspended",
		},
		{
			name:       "refresh invalid",
			err:        entity.ErrRefreshTokenInvalid,
			wantStatus: http.StatusUnauthorized,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if response.Status != tt.wantStatus ||
				response.Code != tt.wantCode ||
				response.RequestID != "request-controller" {
				t.Fatalf("response = %+v", response)
			}

			if strings.Contains(rec.Body.String(), internalErr.Error()) ||
				strings.Contains(rec.Body.String(), "secret_table_key") ||
				strings.Contains(rec.Body.String(), "stack trace") {
				t.Fatalf(
					"internal details leaked: %s",
					rec.Body.String(),
				)
			}
		})
